...
var updateActionInBackand = function (appName, object, action, fileName, userToken, masterToken, token, callback) {
    ...
    return actionConfig.put(appName, object, action, data, null, null, token, masterToken + ":" + userToken)
...
module.exports = function (appName, objectName, actionName, userToken, masterToken, token, folder) {
    ...
    gulp.task('checkRequiredFiles', function(callback){
      ...
      if(!fs.existsSync(sourceFolder + "/index.js")){
        logger.error("Deploy action failed. The action's code was not found in this folder ('" + sourceFolder + "')." +
            " Please set the --folder param to be the full path of the action's code.");
        process.exit(1);
      }

      if(!fs.existsSync(sourceFolder + "/handler.js")){
        logger.error("Deploy action failed. The handler.js file is required in the action's folder ('" + sourceFolder + "').");
        process.exit(1);
      }
      callback();
