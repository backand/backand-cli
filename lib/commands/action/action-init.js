...
module.exports = function (appName, objectName, actionName, userToken, masterToken) {
    ...
    var auth = token ? token : masterToken + ':' + userToken;
    

    gulp.task('isAppExist', function (callback) {
        console.log('isAppExist');
        actionConfig.appExists(appName, null, null, auth).then(function (exists) {
            if (!exists) {
    ...
    gulp.task('isObjectExist', ['isAppExist'], function (callback) {
        ...
        actionConfig.objectExists(appName, objectName, null, null, auth).then(function (exists) {
            if (!exists) {
    ...
    gulp.task('isActionAlreadyExist', ['isObjectExist'], function (callback) {
        console.log('isActionAlreadyExist');
        actionConfig.exists(appName, objectName, actionName, null, null, auth).then(function (exists) {
            if (exists) {
    ...
    gulp.task('createActionFolder', ['isActionAlreadyExist'], function (callback) {
        console.log('createActionFolder');
        createActionFolder(objectName, actionName);
        callback();
    });

    gulp.task('downloadAndSaveAs', ['createActionFolder'], function (callback) {
        console.log('downloadAndSaveAs');
        actionConfig.template(auth).then(function (result) {
