...
var createActionInBackand = function (appName, object, action, userToken, masterToken, token, zipFileName, callback) {

    var data = createrActionDataObject(action, object, zipFileName);
    return actionConfig.post(appName, data, null, null, token, masterToken + ":" + userToken)
...
module.exports = function (appName, objectName, actionName, userToken, masterToken, token, folder, template) {
    ...
    gulp.task('createActionInBackand', ['dist'], function (callback) {
        createActionInBackand(appName, objectName, actionName, userToken, masterToken, token, zipFile, callback);
    });
    ...
    gulp.task('notify', ['clean'], function(callback){
       console.log("Action source code is in folder: " + destFolder);
       callback();
    });
